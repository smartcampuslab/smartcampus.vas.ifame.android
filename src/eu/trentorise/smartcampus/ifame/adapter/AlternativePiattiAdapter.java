...
public class AlternativePiattiAdapter extends ArrayAdapter<Piatto> {

	public AlternativePiattiAdapter(Context context, List<Piatto> piattiList) {
		super(context, android.R.layout.simple_list_item_1, piattiList);
	}

	public AlternativePiattiAdapter(Context context) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (piattoDelGiorno.getPiatto_nome().matches("[0-9]+")) {
			...
			if (num == 1) {
				...
				nome_piatto_del_giorno.setText(getContext().getString(
						R.string.iDeciso_menu_del_giorno_piatti_freddi));
			} else if (num == 4) {
				nome_piatto_del_giorno.setText(getContext().getString(
