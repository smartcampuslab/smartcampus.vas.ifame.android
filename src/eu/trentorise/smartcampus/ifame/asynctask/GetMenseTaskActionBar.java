 ...
 */
public class GetMenseTaskActionBar extends AsyncTask<Void, Void, List<Mensa>> {
	 ...
	 * adapter
	 */
	public GetMenseTaskActionBar(Activity activity, MensaSpinnerAdapter adapter) {
		this.activity = activity;
		this.mensaSpinnerAdapter = adapter;
	...
	protected List<Mensa> doInBackground(Void... params) {
		...
		try {
			...
			Log.e(GetMenseTaskActionBar.class.getName(),
	...
	protected void onPostExecute(List<Mensa> result) {
		...
		if (result == null) {
			...
			mensaSpinnerAdapter.notifyDataSetChanged();
		}
	}
}