...
public class Menu_giorno_tab extends SherlockFragmentActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		dailyMenuTab.setTabListener(new TabListener<MenuGiornoFragment>(this,
				"menuGiornoFragment", MenuGiornoFragment.class,
		...
		alternativeMenuTab.setTabListener(new TabListener<MenuGiornoAlternativeFragment>(this,
				"menuGiornoAlternativeFragment", MenuGiornoAlternativeFragment.class, android.R.id.content));
 ...
 * java.util.Date;
 * 
 * import android.app.TabActivity; import android.content.Intent; import
 * android.graphics.Color; import android.graphics.Typeface; import
 * android.os.Bundle; import android.view.Gravity; import android.view.Menu;
 * import android.view.View; import android.widget.Button; import
 * android.widget.TabHost; import eu.trentorise.smartcampus.ifame.R;
 * 
 ...
 * int year = date.getYear() + 1900; // setTitle(date.getDay() + "-" +
 * date.getMonth() + "/" + year);
 * 
 * SimpleDateFormat s = new SimpleDateFormat("EEEE dd MMMM yyyy"); String
 * daily_menu = s.format(new Date()); //Set the title of the action bar to the
 * current date setTitle(daily_menu);
 * 
 * Intent intent = new Intent(this, Menu_giorno.class); spec =
 * tabHost.newTabSpec
 * ("MENU DEL GIORNO").setIndicator(createTabIndicator("MENU DEL GIORNO"
 * )).setContent(intent); tabHost.addTab(spec);
 * 
 * intent = new Intent(this, Menu_giorno_alternative.class); spec =
 * tabHost.newTabSpec
 * ("ALTERNATIVE").setIndicator(createTabIndicator("ALTERNATIVE"
 * )).setContent(intent); tabHost.addTab(spec); tabHost.setCurrentTab(0);
 * 
 * 
 * 
 * }
 ...
 * getMenuInflater().inflate(R.menu.menu_giorno_tab, menu); return true; }
 * 
 * 
 * 
 * //per eventuali modifiche di layout del tab agire qui private View
 * createTabIndicator(String text) { Button button = new Button(this);
 * button.setBackgroundResource(R.color.sc_dark_gray);
 * button.setTypeface(null,Typeface.BOLD); button.setTextSize(12);
